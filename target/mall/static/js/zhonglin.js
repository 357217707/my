...
//轮播图 END
	...
	$(".mid-sl .sl-right").click(function(event) {
		...
		if (vl>0) {
            ...
            $(this).siblings("input").val(vl);
            var id = $(this).siblings("input").attr("sign")

            var price = $("p[name=price_"+id+"]").children("i").text();
            var total = price * vl;
            $("p[name=priceAll_"+id+"]").children("i").text(total.toFixed(2));
            updateNum(id,vl);
		}
	});
    ...
    function updateNum(id, num) {
        $.post("/cart/update", { num: num,id:id});
    }


    /*购物车数量添加*/
	$(function () {
        $(".mid-sl .sl-left").each(function () {
            var vl = $(this).siblings("input").val();
            $(this).siblings("input").val(vl);
            var id = $(this).siblings("input").attr("sign")
